...
class MultiScaleDeformableAttention(nn.Module):
    ...
    def __init__(self, embed_dim = 256, num_heads = 8, num_levels = 4, num_points = 4, img2col_step = 64,
                 batch_first = False):
        
        # Inherit from parent
        super().__init__()
        
        # Check for correct variable proportions
        if embed_dim % num_heads != 0:
            err_msg = "embed_dim ({}) must be divisible by num_heads ({})".format(embed_dim, num_heads)
    ...
    def forward(
        ...
        **kwargs
        ...
        # In original code base, there is a cuda-compiled variant of ms. deformable attention
        # -> Has been removed for now, since it requires more complex compilation setup
